...
export class MainView extends LitElement {
    ...
    return html`
      <vaadin-app-layout primary-section="drawer">
        ...
        <div slot="drawer">
          ...
          <div class="copyright">© your company, 2020</div>
        </div>
        <slot></slot>
      </vaadin-app-layout>
    `;
  ...
  private isCurrentLocation(route: string): boolean {
    let url = this.location.getUrl();
    if (url.indexOf('/portfolio') !== -1) {
      url = url.substr(0, url.indexOf('/portfolio')) + '/portfolio';
    }

    return router.urlForPath(route) === url;
  }
